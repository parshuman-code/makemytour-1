...
import { Plane, Clock, ShieldCheck, Star, Users, TrendingUp, Lock, Sparkles, CheckCircle2, Heart } from 'lucide-react';
...
export default function FlightCard({ flight, onBookFlight, user, setUser }) {
  ...
  const flightItemId = flightId;
  const [isSaved, setIsSaved] = useState(() => isItemSaved(user, flightItemId, 'flight'));

  const handleToggleSave = async () => {
    const { isSaved: newState } = await toggleSavedItem(
      user, setUser,
      flightItemId, 'flight',
