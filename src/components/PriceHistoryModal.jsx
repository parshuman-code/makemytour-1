  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* Modal Header */}
        <div className="p-6 bg-blue-50 text-slate-800 flex items-start justify-between relative overflow-hidden border-b border-blue-100">
          <div className="relative z-10">
            <div className="flex items-center gap-2 text-xs font-semibold text-blue-600 uppercase tracking-widest mb-1">
              <TrendingUp className="w-4 h-4" />
              Fare Trend History
            </div>
            <h3 className="text-xl font-black flex items-center gap-2 text-blue-950">
              <span>{flight.airline || flight.flightName || 'SkyHigh'}</span>
              <span className="text-slate-500 font-normal text-sm">({flight.flightNumber || flight.code || 'FL-202'})</span>
            </h3>
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-full bg-white hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors relative z-10 shadow-sm border border-slate-200"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
