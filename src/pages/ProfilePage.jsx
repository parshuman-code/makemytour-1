...
import { User, Phone, Mail, Edit2, MapPin, Calendar, CreditCard, X, Check, LogOut, Plane, Building2, Lock, Clock, ArrowRight, Trash2, ShieldAlert, RefreshCw, AlertTriangle, Heart, Search } from 'lucide-react';
...
export default function ProfilePage({ user, setUser, onLogout }) {
  ...
  const [savedItems, setSavedItems] = useState(user?.savedItems || []);
  const [recentSearches, setRecentSearches] = useState(user?.recentSearches || []);
        ...
        fetch(`/api/user/email?email=${encodeURIComponent(user.email)}`)
          ...
          .then(freshUser => {
            if (freshUser && freshUser._id) {
              ...
              setSavedItems(freshUser.savedItems || []);
              setRecentSearches(freshUser.recentSearches || []);
            }
          })
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {savedItems.map((item, idx) => (
                  <div key={idx} className="border border-slate-200 rounded-xl p-4 flex items-center gap-3 hover:shadow-sm transition-shadow">
                    {item.image ? (
                      <img src={item.image} alt={item.title} className="w-14 h-14 rounded-xl object-cover flex-shrink-0" />
                    ) : (
                      <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 ${item.type === 'hotel' ? 'bg-emerald-50' : 'bg-blue-50'}`}>
                        {item.type === 'hotel' ? <Building2 className="w-6 h-6 text-emerald-500" /> : <Plane className="w-6 h-6 text-blue-500" />}
                      </div>
                    )}
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-slate-900 text-sm truncate">{item.title}</p>
                      <p className="text-xs text-slate-500 capitalize">{item.type}</p>
                      {item.price && <p className="text-sm font-black text-slate-900">₹{Number(item.price).toLocaleString('en-IN')}</p>}
                    </div>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${item.type === 'hotel' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>
                      {item.type}
                    </span>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="flex flex-wrap gap-3">
                {recentSearches.map((s, idx) => (
                  <div key={idx} className="inline-flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-full px-4 py-2 text-sm">
                    {s.type === 'hotel' ? <Building2 className="w-3.5 h-3.5 text-emerald-500" /> : <Plane className="w-3.5 h-3.5 text-blue-500" />}
                    <span className="font-semibold text-slate-700">{s.query}</span>
                    <span className="text-slate-400 text-xs">{new Date(s.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</span>
                  </div>
                ))}
              </div>
            </div>
