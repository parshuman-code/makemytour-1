...
import { 
  ...
  MapPin, Calendar, Users, QrCode, Sparkles, Flame, ShieldCheck, Lock, TrendingUp, Heart
} from 'lucide-react';
...
import { saveSearchToAccount, toggleSavedItem, isItemSaved } from '../utils/userSync';

export default function HomePage({ flights = [], bestOffers, onBookFlight, user, setUser }) {
  ...
  const handlesearch = () => {
    if (bookingtype === "flights") {
      ...
      // Persist search to user account
      if (from || to) {
        saveSearchToAccount(
          user,
          `${from || 'Any'} → ${to || 'Any'}`,
      ...
      if (to) {
        ...
        setsearchresults(results);
        saveSearchToAccount(user, to, 'hotel', { city: to, date, travelers });
      } else {
        // If no city is entered, show all hotels
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-xl shadow-lg mx-auto max-w-5xl p-6">
          ...
          {/* Smart Fare Adjuster Interactive Toggle Banner */}
          {bookingtype === "flights" && (
            <div className="mt-6 p-4 rounded-2xl bg-white text-slate-800 flex flex-wrap items-center justify-between gap-4 border border-blue-100 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xs font-extrabold text-blue-800 uppercase tracking-widest flex items-center gap-1.5">
                    <span>Smart Fare Adjuster</span>
                    <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                    <span className="text-[10px] text-emerald-600 font-bold lowercase">live</span>
                  </h4>
                  <p className="text-xs text-slate-500 font-semibold mt-0.5">
                    Our smart system checks seat demand, holidays, and dates to find you the most accurate price.
                  </p>
                </div>
              </div>

              <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-200">
                <span className="text-xs font-bold text-slate-600">Preview Holiday Prices (+20%)</span>
                <button
                  onClick={() => setIsHolidayPeak(!isHolidayPeak)}
                  className={`w-12 h-6 flex items-center rounded-full p-1 transition-colors duration-300 ${
                    isHolidayPeak ? 'bg-blue-600 justify-end' : 'bg-slate-300 justify-start'
                  }`}
                >
                ...
                {searchresults.length > 0 ? (
                  searchresults.map((result) => (
                    <HotelSearchCard
                      ...
                      user={user}
                      setUser={setUser}
                      onBook={() => onBookFlight({ ...result, isHotel: true }, travelers)}
                    />
                  ))
                ) : (
                  <div className="text-gray-500 text-sm py-8 col-span-3 text-center">
...
function HotelSearchCard({ hotel, user, setUser, onBook }) {
  ...
  const handleSave = async () => {
    const { isSaved } = await toggleSavedItem(
      user, setUser,
      itemId, 'hotel',
      hotel.hotelName,
      hotel.pricePerNight,
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col">
      {hotel.image && (
        <div className="relative h-40 overflow-hidden">
          ...
          {user && (
            <button
              onClick={handleSave}
              className={`absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center shadow transition-colors ${saved ? 'bg-rose-500 text-white' : 'bg-white/90 text-slate-400 hover:text-rose-500'}`}
      ...
      <div className="p-4 flex flex-col flex-1 justify-between">
        <div>
          <h3 className="font-extrabold text-base text-slate-900 mb-0.5 leading-tight">{hotel.hotelName}</h3>
          <p className="text-xs text-slate-500 font-medium mb-2">{hotel.location}</p>
          <p className="text-lg font-black text-slate-900">
            {String.fromCharCode(8377)}{Number(hotel.pricePerNight).toLocaleString('en-IN')}
            <span className="text-xs font-semibold text-slate-400 ml-1">/ night</span>
          </p>
        </div>
        <button
          className="mt-3 w-full bg-slate-900 hover:bg-black text-white font-bold py-2 rounded-lg text-sm transition-colors"
          onClick={onBook}
        >
          Book Now
        </button>
